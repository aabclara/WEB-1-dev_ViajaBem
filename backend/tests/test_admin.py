import pytest
from app.infra.modelos import TipoUsuario, StatusReserva
...
class TestAdmin:
    async def test_listar_viagens_admin_vazia(self, cliente, sessao_teste):
        ...
        r = await cliente.get(
            "/admin/viagens/", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    async def test_criar_e_editar_viagem(self, cliente, sessao_teste):
        ...
        r = await cliente.patch(
            f"/admin/viagens/{viagem_id}",
            json={"vagas_totais": 50},
            headers={"Authorization": f"Bearer {token}"},
        )
        assert r.status_code == 200
    ...
    async def test_kanban_reservas(self, cliente, sessao_teste):
        ...
        r = await cliente.post(
            "/admin/viagens/",
            json={"titulo": "V1", "data_partida": "2026-12-01", "vagas_totais": 10},
            headers={"Authorization": f"Bearer {token}"},
        )
        vid = r.json()["id"]
    ...
    async def test_exportar_antt(self, cliente, sessao_teste):
        ...
        r = await cliente.post(
            "/admin/viagens/",
            json={"titulo": "V_CSV", "data_partida": "2026-12-01", "vagas_totais": 10},
            headers={"Authorization": f"Bearer {token}"},
        )
        vid = r.json()["id"]
        ...
        # Exportar JSON
        r = await cliente.get(
            f"/admin/viagens/{vid}/exportar-passageiros",
            headers={"Authorization": f"Bearer {token}"},
        )
        assert r.status_code == 200
        ...
        # Exportar CSV
        r = await cliente.get(
            f"/admin/viagens/{vid}/exportar-passageiros?formato=csv",
            headers={"Authorization": f"Bearer {token}"},
        )
        assert r.status_code == 200
    ...
    async def test_resumo_whatsapp_erro_sem_valor(self, cliente, sessao_teste):
        ...
        reserver = ReservaGrupo(
            id_viagem=v.id, id_lider=u.id, qtd_vagas=2, valor_acordado=None
        )
        ...
        r = await cliente.get(
            f"/admin/reservas/{reserver.id}/resumo-whatsapp",
            headers={"Authorization": f"Bearer {token}"},
        )
    ...
    async def test_resumo_whatsapp_sucesso(self, cliente, sessao_teste):
        ...
        reserver = ReservaGrupo(
            id_viagem=v.id, id_lider=u.id, qtd_vagas=2, valor_acordado=100.0
        )
        sessao_teste.add(reserver)
        await sessao_teste.commit()
    ...
    async def test_listar_viagens_busca(self, cliente, sessao_teste):
        ...
        sessao_teste.add(
            Viagem(titulo="Floripa", data_partida=date(2026, 1, 1), vagas_totais=10)
        )
        sessao_teste.add(
            Viagem(titulo="Natal", data_partida=date(2026, 1, 1), vagas_totais=10)
        )
        await sessao_teste.commit()

        r = await cliente.get(
            "/admin/viagens/?busca=Floripa",
            headers={"Authorization": f"Bearer {token}"},
        )
        assert len(r.json()) == 1
