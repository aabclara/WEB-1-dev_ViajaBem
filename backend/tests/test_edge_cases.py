...
class TestEdgeCases:
    ...
    async def test_resumo_financeiro_reserva_inexistente(self, cliente, sessao_teste):
        ...
        r = await cliente.get(
            "/reservas/9999/resumo", headers={"Authorization": f"Bearer {tk}"}
        )
        assert r.status_code == 404
    ...
    async def test_resumo_whatsapp_nao_encontrado_admin(self, cliente, sessao_teste):
        await _criar_usuario(
            sessao_teste,
            email="edge_admin2@test.com",
            tipo=TipoUsuario.ADMIN,
            cpf="edge3",
        )
        tk = await _token(cliente, "edge_admin2@test.com")
        r = await cliente.get(
            "/admin/reservas/9999/resumo-whatsapp",
            headers={"Authorization": f"Bearer {tk}"},
        )
        assert r.status_code == 404

    async def test_exportar_passageiros_viagem_nao_encontrada(
        self, cliente, sessao_teste
    ):
        await _criar_usuario(
            sessao_teste,
            email="edge_admin3@test.com",
            tipo=TipoUsuario.ADMIN,
            cpf="edge4",
        )
        tk = await _token(cliente, "edge_admin3@test.com")
        r = await cliente.get(
            "/admin/viagens/9999/exportar-passageiros",
            headers={"Authorization": f"Bearer {tk}"},
        )
        assert r.status_code == 404
    ...
    async def test_kanban_viagem_nao_encontrada(self, cliente, sessao_teste):
        await _criar_usuario(
            sessao_teste,
            email="edge_admin4@test.com",
            tipo=TipoUsuario.ADMIN,
            cpf="edge5",
        )
        tk = await _token(cliente, "edge_admin4@test.com")
        r = await cliente.get(
            "/admin/viagens/9999/reservas", headers={"Authorization": f"Bearer {tk}"}
        )
        assert r.status_code == 404
    ...
    async def test_obter_reserva_propria(self, cliente, sessao_teste):
        ...
        r = await cliente.get(
            f"/reservas/{reserva.id}", headers={"Authorization": f"Bearer {tk2}"}
        )
