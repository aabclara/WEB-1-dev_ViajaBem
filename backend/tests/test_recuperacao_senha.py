...
class TestRecuperacaoSenha:
    async def test_fluxo_esqueci_senha_e_redefinicao(self, cliente, sessao_teste):
        # 1. Criar usuário
        usuario = await _criar_usuario(
            sessao_teste, email="reset@test.com", cpf="99988877766"
        )
        ...
        res = await sessao_teste.execute(
            select(TokenRedefinicaoSenha).where(
                TokenRedefinicaoSenha.id_usuario == usuario.id
            )
        )
        ...
        # 4. Redefinir senha
        payload = {"token": token_str, "nova_senha": "NovaSenha123!"}
        ...
        r = await cliente.post(
            "/auth/login",
            data={"username": "reset@test.com", "password": "NovaSenha123!"},
        )
        assert r.status_code == 200
        assert "access_token" in r.json()

    async def test_redefinir_senha_token_invalido(self, cliente, sessao_teste):
        payload = {"token": "token-que-nao-existe", "nova_senha": "SenhaInvalida123!"}
        r = await cliente.post("/auth/redefinir-senha", json=payload)
    ...
    async def test_esqueci_senha_usuario_inexistente(self, cliente):
        ...
        r = await cliente.post(
            "/auth/esqueci-senha", json={"email": "naoexiste@test.com"}
        )
        assert r.status_code == 200
