...
class TestLogin:
    async def test_login_valido(self, cliente, sessao_teste):
        ...
        await _criar_usuario(
            sessao_teste, email="loginvalido@test.com", cpf="66666666666"
        )
        r = await cliente.post(
            "/auth/login",
            data={"username": "loginvalido@test.com", "password": "Senha123!"},
        )
    ...
    async def test_login_senha_invalida(self, cliente, sessao_teste):
        ...
        await _criar_usuario(
            sessao_teste, email="loginsenha@test.com", cpf="77777777777"
        )
        r = await cliente.post(
            "/auth/login",
            data={"username": "loginsenha@test.com", "password": "errada"},
        )
    ...
    async def test_login_usuario_inexistente(self, cliente):
        r = await cliente.post(
            "/auth/login",
            data={"username": "naoexiste@test.com", "password": "Senha123!"},
        )
        assert r.status_code == 401
...
class TestRBAC:
    async def test_lider_nao_acessa_rota_admin(self, cliente, sessao_teste):
        ...
        await _criar_usuario(
            sessao_teste, email="lider_rbac@test.com", cpf="88888888888"
        )
        tk = await _token(cliente, "lider_rbac@test.com")
        r = await cliente.get(
            "/admin/viagens/", headers={"Authorization": f"Bearer {tk}"}
        )
