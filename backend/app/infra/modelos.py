...
class Usuario(Base):
    ...
    tipo: Mapped[TipoUsuario] = mapped_column(
        SAEnum(TipoUsuario, name="tipo_usuario"), nullable=False
    )
...
class Viagem(Base):
    ...
    status: Mapped[StatusViagem] = mapped_column(
        SAEnum(StatusViagem, name="status_viagem"),
        nullable=False,
        default=StatusViagem.ATIVO,
    )
    criado_em: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
    ...
    reservas: Mapped[list["ReservaGrupo"]] = relationship(
        "ReservaGrupo", back_populates="viagem"
    )
...
class ReservaGrupo(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    id_viagem: Mapped[int] = mapped_column(
        ForeignKey("viagem.id", ondelete="CASCADE"), nullable=False
    )
    ...
    admin_responsavel_id: Mapped[int | None] = mapped_column(
    ...
    lider: Mapped["Usuario"] = relationship(
        "Usuario", foreign_keys=[id_lider], back_populates="reservas"
    )
    admin_responsavel: Mapped["Usuario | None"] = relationship(
        "Usuario", foreign_keys=[admin_responsavel_id]
    )
    passageiros: Mapped[list["Passageiro"]] = relationship(
        "Passageiro", back_populates="reserva"
    )
...
class Passageiro(Base):
    ...
    id_reserva: Mapped[int] = mapped_column(
        ForeignKey("reserva_grupo.id", ondelete="CASCADE"), nullable=False
    )
    ...
    reserva: Mapped["ReservaGrupo"] = relationship(
        "ReservaGrupo", back_populates="passageiros"
    )
