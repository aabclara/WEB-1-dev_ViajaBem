...
async def listar_viagens(sessao: AsyncSession = Depends(obter_sessao)):
    ...
    await sessao.execute(
        ...
        .where(
            ...
            ViagemModel.status.not_in(
                [StatusViagem.FINALIZADO, StatusViagem.CANCELADO]
            ),
        )
    ...
    resultado = await sessao.execute(
        select(ViagemModel)
        .where(ViagemModel.status == StatusViagem.ATIVO)
        .order_by(ViagemModel.data_partida)
    )
    ...
    for v in viagens:
        ...
        resposta.append(
            ViagemPublicaSchema(
                id=v.id,
                titulo=v.titulo,
                descricao_precos=v.descricao_precos,
                descricao_curta=v.descricao_curta,
                itens_inclusos=v.itens_inclusos,
...
async def detalhe_viagem(id_viagem: int, sessao: AsyncSession = Depends(obter_sessao)):
    ...
    return ViagemPublicaSchema(
        ...
        descricao_precos=viagem.descricao_precos,
        descricao_curta=viagem.descricao_curta,
        itens_inclusos=viagem.itens_inclusos,
