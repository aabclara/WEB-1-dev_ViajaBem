...
from app.core.seguranca import requerer_lider, obter_usuario_atual
...
@roteador_reservas.post(
    "/", response_model=ReservaSchema, status_code=status.HTTP_201_CREATED
)
...
async def obter_reserva(
    ...
    if reserva_model.passageiros:
        dominio_reserva.passageiros = [
            MapeadorPassageiro.para_dominio(p) for p in reserva_model.passageiros
        ]
...
@roteador_reservas.get(
    "/{id_reserva}/link-acompanhante", response_model=AcompanhanteSchema
)
async def link_acompanhante(
    ...
    return AcompanhanteSchema(
        ...
        data_partida=reserva_model.viagem.data_partida
        if reserva_model.viagem
        else None,
...
@roteador_reservas.get(
    "/{id_reserva}/passageiros", response_model=list[PassageiroSchema]
)
