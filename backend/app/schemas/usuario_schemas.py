...
class CriarUsuarioSchema(BaseModel):
    email: EmailStr = Field(
        ..., description="E-mail válido", examples=["usuario@email.com"]
    )
    senha: str = Field(
        ..., description="Senha forte do usuário", examples=["S3nh@F0rt3"]
    )
    nome: str = Field(..., description="Nome completo", examples=["Maria Souza"])
    apelido: Optional[str] = Field(
        None, description="Apelido opcional", examples=["Mari"]
    )
    cpf: str = Field(
        ..., description="CPF válido de 11 dígitos", examples=["12345678900"]
    )
    telefone: str = Field(..., description="Telefone com DDD", examples=["11999999999"])
    data_nascimento: date = Field(
        ..., description="Data de nascimento", examples=["1995-10-25"]
    )
    tipo: TipoUsuario = Field(
        ..., description="Tipo de acesso no sistema", examples=["LIDER"]
    )
    ...
    def validar_tipo(cls, v: TipoUsuario) -> TipoUsuario:
        ...
        if v == TipoUsuario.ADMIN:
            raise ValueError(
                "Não é permitido cadastrar usuários ADMIN via este endpoint"
            )
        return v
...
class LoginSchema(BaseModel):
    email: EmailStr = Field(
        ..., description="E-mail de acesso", examples=["usuario@email.com"]
    )
...
class TokenSchema(BaseModel):
    ...
    nome: str = Field(
        ..., description="Nome do dono do token", examples=["Maria Souza"]
    )
    apelido: Optional[str] = Field(None, description="Apelido do usuário")
    email: str = Field(
        ..., description="Email verificado", examples=["maria@email.com"]
    )
...
class EsqueciSenhaSchema(BaseModel):
    email: EmailStr = Field(
        ..., description="E-mail para recuperação", examples=["usuario@email.com"]
    )
...
class UsuarioPublicoSchema(BaseModel):
    ...
    email: str = Field(
        ..., description="Endereço de e-mail", examples=["maria@email.com"]
    )
